...
class AuthService {
  ...
  userStore = async (data) => {
    try {
      ...
      return await user.save(); // insert operation
    } catch (exception) {
      throw exception;
    }
  };
  ...
  updateSingleUserByFilter = async (filter, updateData) => {
    try {
      let update = await User.findOneAndUpdate(
        ...
        { $set: updateData },
        { new: true }
      ); // the new: true option means that the function will return the updated document rather than the original one before the update.
      return update;
    } catch (exception) {
      throw exception;
    }
  ...
  notifyActivationEmal = async ({ name, email, activationToken }) => {
    try {
      ...
      let msg = `
            ...
            ${appConfig.frontendUrl}/activate/${activationToken}</a>
            <br>
  ...
  notifyForgetPasswordEmail = async ({ name, email, forgetPaswwordToken }) => {
    try {
      let msg = `
            ...
            <a href="${appConfig.frontendUrl}/verify-forget-token/${forgetPaswwordToken}" style="color: #008000; text-decoration:underline;">
